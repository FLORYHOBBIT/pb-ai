...
- 根据版本号动态解析 ORCA、编译器、VM 和库管理 DLL 名称，不再在加载流程里写死 PB8。
- `src/native/runtime_profiles.h` 保存已验证构建号及回调字段布局；PB8 写入字段从 0x40 开始，PB9 从 0x44 开始。匹配实际已加载模块版本后才启用内部回调。
- 编译、PBD 写入、EXE 写入分别检测能力。CLI 显示实际运行库目录和三项能力；缺少进度能力不会额外预编译。
- PB8 8.0.2.9506、PB9 9.0.3.8836：中文资源路径、完整构建、PBD 生成、EXE 内嵌对象写入、生成 EXE 运行均通过。共 9 项单元测试、5 项原生测试通过。
- PB12.5.2.5629：独立基础库操作出现 0xC0000005，尚未解决，未加入已验证进度配置表。不能将本版本描述为所有 PB 版本均已验证。可设置 PB_TEST_VERSIONS=125 后运行 test:native 复现相关构建测试。
- 修正 Unicode 库目录注释缓冲区按字符数分配时的字节容量，但这没有消除上述 PB12.5 异常。